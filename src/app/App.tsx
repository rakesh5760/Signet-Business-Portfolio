...
export default function App() {
  ...
  const scrollToSection = (id: string) => {
    ...
    if (element) {
      // Use requestAnimationFrame to ensure the scroll is registered 
      // before any React state updates might interfere.
      requestAnimationFrame(() => {
        element.scrollIntoView({ behavior: 'smooth', block: 'start' });
      });

      // CRITICAL FIX: Delay closing the menu to ensure the smooth scroll 
      // has 'locked on' before the drawer unmounts.
      setTimeout(() => {
        setIsMenuOpen(false);
      }, 250);
    }
